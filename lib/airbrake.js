...
function Airbrake() {
  ...
  this.options =  {
    ...
    serviceEndPoint: '/notifier_api/v2/notices',
...
Airbrake.createClient = function(key, options) {
  var instance = new this();
  instance.key = key;
  instance.options = hashish.merge(instance.options, options);
  return instance;
};
...
Airbrake.prototype.notify = function(err, cb) {
  ...
  if (this.developmentEnvironments.indexOf(this.options.env) != -1) {
    this.log(err);
  ...
  var options = {
    method: 'POST',
    url: this.url(this.options.serviceEndPoint),
    body: body,
  ...
  request(options, function(err, res, body) {
    ...
    if (res.statusCode >= 300) {
      ...
      explanation = (explanation) ? ': ' + explanation[1] : ': ' + body;
...
Airbrake.prototype.url = function(path) {
  if(this.options.secure){
    return 'https://' + this.options.serviceHost + path;
  }
  return 'http://' + this.options.serviceHost + path;
};
...
Airbrake.prototype.sessionVars = function(err) {
  return (err.session instanceof Object) ? err.session : {};
};

Airbrake.prototype.paramsVars = function(err) {
  return (err.params instanceof Object) ? err.params : {};
};
